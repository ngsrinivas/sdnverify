...
var
 switches: array [SwitchID] of SwitchType;
...
procedure installFlow(inPort: PortID; outPort:PortID; pkt: PacketHeaderType; switchID: SwitchID);
var srcDeviceMac, dstHostMac: MacAddress;
var nextAvailableMicroFlowID: 0..(NUMMICROFLOWS-1);
begin
dstHostMac := getDstHostMac(pkt);
nextAvailableMicroFlowID := switches[switchID].flowTable[inPort][outPort].nextAvailableMicroFlowID;

/*
if(switches[switchID].flowTable[inPort][outPort].src != srcDeviceMac) then
  put "src is: "; put switches[switchID].flowTable[inPort][outPort].src; put srcDeviceMac; -- sanity check
  assert false;
endif;
*/

switches[switchID].flowTable[inPort][outPort].microFlows[nextAvailableMicroFlowID].dst := dstHostMac;
switches[switchID].flowTable[inPort][outPort].microFlows[nextAvailableMicroFlowID].action := FORWARD;
switches[switchID].flowTable[inPort][outPort].nextAvailableMicroFlowID := nextAvailableMicroFlowID + 1;
endProcedure;

function getOutPortFromFlow(inPacket:PacketHeaderType; inPort: PortID; switchID: SwitchID): PortID;
begin
for guessedOutPort: PortID do
      for microFlowID: MicroFlowID do
	 if (switches[switchID].flowTable[inPort][guessedOutPort].microFlows[microFlowID].dst = getDstHostMac(inPacket) & 
	     switches[switchID].flowTable[inPort][guessedOutPort].microFlows[microFlowID].action = FORWARD)
	then 
...
-- do appropriate action which is essentially forwarding or flooding.
*/
ruleset switchID: SwitchID; linkID: LinkID; randPacketType: PacketType; randDstHostID: HostID; randSrcHostID: HostID; randPacketTag: PacketTag; randPortID: PortID
...
--checkPktIntegrity(randPacketType, randSrcDeviceType, randSrcDeviceSwitchID, randSrcDeviceHostID, randDstHostID, randSrcHostID, randPacketTag, randPortID, currSwi
--tchID, currLinkID) & (randPacketTag = env)
 ==>
  var inPacket,generatedPacket: PacketHeaderType;
  --var srcMac: MacAddress;
  var inPort, outPort: PortID;
...
begin
  ...
  --inPacket := packet; 
  --inPacket := generatePacket(randPacketType, randSrcDeviceMac, randDstHostMac, randSrcHostMac, randPacketTag);
inPacket := generatePacket(randPacketType, getMac(randDstHostID,HOST,0), getMac(randSrcHostID,HOST,0), randPacketTag);
--  srcMac := getSrcDeviceMac(inPacket);
  inPort := getPortID(linkID, switchID);
  isMatching := isThereMatchingMicroFlow(inPacket, inPort, switchID);
  if(isMatching) then -- i.e. if flowtable has entry for the packet
     outPort := getOutPortFromFlow(inPacket, inPort, switchID);
    ...
    if (secureChannels[switchID].status = noReq) then --i.e. keep waiting for controller to become empty
     sendController(inPacket, inPort, switchID); -- through secure channel    
     ------removePacketFromLink(linkID);
    endif;
  endif;
