...
begin
  ...
  if switchID = 1 then 
    ...
    if getSrcHostID(pkt) = 1 then
-- ack part needed since a req packet from host 1 getting blocked is correct behavior
-- and environ part needed since we are checking that for any interference, a concrete packet which is replied by the Host 1
      -- never results in the host getting blocked. Then by symmetry, this is always true.
...
invariant "noPersistentBlockage"
 ...
 --ctrlStateExternalHostContacted -> (droppedPacketCount < 1);
